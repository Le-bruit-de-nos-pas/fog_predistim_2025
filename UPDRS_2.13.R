...
UPDRSII <- UPDRSII %>% select(SUBJID, VISIT, MDS2_13OFF, MDS2_13ON)

UPDRSII <- UPDRSII[-1,]

UPDRSII <- UPDRSII %>% filter(VISIT=="Visite de screening") %>% select(-VISIT) %>%
  rename("V0_MDS2_13OFF"="MDS2_13OFF", "V0_MDS2_13ON"="MDS2_13ON") %>%
  full_join(UPDRSII %>% filter(VISIT=="Visite Bilan à 1 an - V1") %>% select(-VISIT) %>%
  rename("V1_MDS2_13OFF"="MDS2_13OFF", "V1_MDS2_13ON"="MDS2_13ON"))


UPDRSII <- UPDRSII %>% mutate(V0_MDS2_13OFF=ifelse(V0_MDS2_13OFF=="Normal",0,
                                        ifelse(V0_MDS2_13OFF=="Minime",1,
                                               ifelse(V0_MDS2_13OFF=="Léger", 2,
                                                      ifelse(V0_MDS2_13OFF=="Modéré",3,
                                                             ifelse(V0_MDS2_13OFF=="Sévère",4,NA)))))) %>%
   mutate(V0_MDS2_13ON=ifelse(V0_MDS2_13ON=="Normal",0,
                                        ifelse(V0_MDS2_13ON=="Minime",1,
                                               ifelse(V0_MDS2_13ON=="Léger", 2,
                                                      ifelse(V0_MDS2_13ON=="Modéré",3,
                                                             ifelse(V0_MDS2_13ON=="Sévère",4,NA)))))) %>%
   mutate(V1_MDS2_13OFF=ifelse(V1_MDS2_13OFF=="Normal",0,
                                        ifelse(V1_MDS2_13OFF=="Minime",1,
                                               ifelse(V1_MDS2_13OFF=="Léger", 2,
                                                      ifelse(V1_MDS2_13OFF=="Modéré",3,
                                                             ifelse(V1_MDS2_13OFF=="Sévère",4,NA)))))) %>%
   mutate(V1_MDS2_13ON=ifelse(V1_MDS2_13ON=="Normal",0,
                                        ifelse(V1_MDS2_13ON=="Minime",1,
                                               ifelse(V1_MDS2_13ON=="Léger", 2,
                                                      ifelse(V1_MDS2_13ON=="Modéré",3,
                                                             ifelse(V1_MDS2_13ON=="Sévère",4,NA))))))


UPDRSII <- data.frame(UPDRSII) %>% mutate_each(as.numeric, V0_MDS2_13OFF :V1_MDS2_13ON)


UPDRSII_V0 <- UPDRSII %>% select(SUBJID, V0_MDS2_13OFF, V0_MDS2_13ON) %>% drop_na() 

UPDRSII_V1 <- UPDRSII %>% select(SUBJID, V1_MDS2_13OFF, V1_MDS2_13ON) %>% drop_na() 

UPDRSII_OFF <- UPDRSII %>% select(SUBJID, V0_MDS2_13OFF, V1_MDS2_13OFF) %>% drop_na() 

UPDRSII_ON <- UPDRSII %>% select(SUBJID, V0_MDS2_13ON, V1_MDS2_13ON) %>% drop_na() 
...
fwrite(UPDRSII_V0, "data/UPDRSII_V0_2.13.txt")
fwrite(UPDRSII_V1, "data/UPDRSII_V1_2.13.txt")
fwrite(UPDRSII_OFF, "data/UPDRSII_OFF_2.13.txt")
fwrite(UPDRSII_ON, "data/UPDRSII_ON_2.13.txt")
fwrite(UPDRSII, "data/UPDRSII_2.13.txt")
...
data_long <- UPDRSII %>%
  pivot_longer(cols = c(V0_MDS2_13OFF, V0_MDS2_13ON, V1_MDS2_13OFF, V1_MDS2_13ON),
               names_to = "Condition",
               values_to = "Value")
...
# Friedman chi-squared = 189.65, df = 3, p-value < 2.2e-16
...
# 
#               V0_MDS2_13OFF V0_MDS2_13ON V1_MDS2_13OFF
# V0_MDS2_13ON  < 2e-16       -            -            
# V1_MDS2_13OFF 1             3.9e-13      -            
# V1_MDS2_13ON  3.0e-15       1            < 2e-16      
...
plot1 <- summary_stats %>% mutate(Condition=ifelse(Condition=="V0_MDS2_13OFF", "V0 2.13 OFF",
                                          ifelse(Condition=="V0_MDS2_13ON", "V0 2.13 ON",
                                                 ifelse(Condition=="V1_MDS2_13OFF", "V1 2.13 OFF", "V1 2.13 ON")))) %>%
  mutate(Condition=factor(Condition, levels=c("V0 2.13 OFF", "V0 2.13 ON","V1 2.13 OFF", "V1 2.13 ON"))) %>%
...
percentage_stats <- percentage_stats %>% mutate(Condition=ifelse(Condition=="V0_MDS2_13OFF", "V0 2.13 OFF",
                                          ifelse(Condition=="V0_MDS2_13ON", "V0 2.13 ON",
                                                 ifelse(Condition=="V1_MDS2_13OFF", "V1 2.13 OFF", "V1 2.13 ON")))) %>%
  mutate(Condition=factor(Condition, levels=c("V0 2.13 OFF", "V1 2.13 OFF", "V0 2.13 ON", "V1 2.13 ON"))) 
